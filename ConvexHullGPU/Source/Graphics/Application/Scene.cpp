...
Scene::Scene():
	_cameraManager(std::unique_ptr<CameraManager>(new CameraManager())), _modelCompSelected(-1), _nextFramebufferID(0), _ssaoFBO(new SSAOFBO())
{
...
void Scene::computeCollisions()
{
	std::vector<Model3D::FaceGPUData> collidedFaces;
	
	for (int groupIdx1 = 0; groupIdx1 < _sceneGroup.size(); ++groupIdx1)
	{
		for (int groupIdx2 = 0; groupIdx2 < _sceneGroup.size(); ++groupIdx2)
		{
			if (groupIdx1 != groupIdx2)
			{
				_sceneGroup[groupIdx1]->hasCollided(_sceneGroup[groupIdx2]);
			}
		}
	}
}
...
bool Scene::hit(Model3D::RayGPUData& ray)
{
	float minDistance = FLT_MAX, distance;
	GLuint modelComp = 0;
	_modelCompSelected = -1;			// Reset in case there is no collision
	
	for (int groupIdx = 0; groupIdx < _sceneGroup.size(); ++groupIdx)
	{
		if (_sceneGroup[groupIdx]->hit(ray, distance, modelComp) && distance < minDistance)
		{
			minDistance = distance;
			_modelCompSelected = modelComp;
		}
	}

	return _modelCompSelected != -1;
}
...
void Scene::drawAsTriangles(Camera* camera, const mat4& mModel, RenderingParameters* rendParams)
{
...

	{
		for (unsigned int i = 0; i < _lights.size(); ++i)																// Multipass rendering
		{
			...
			shader->applyActiveSubroutines();
			shader->setUniform("selectedModelComponent", _modelCompSelected);
