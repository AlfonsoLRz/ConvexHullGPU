...
bool Group3D::hasCollided(Group3D* group)
{
	...
	const GLuint faceSSBO					= ComputeShader::setWriteBuffer(Model3D::FaceGPUData(), _staticGPUData->_numTriangles, GL_DYNAMIC_DRAW);
	...
	std::cout << ChronoUtilities::getDuration() / float(numCollisions) << std::endl;
	
	FaceGPUData* faces		= ComputeShader::readData(faceSSBO, FaceGPUData());	

	_triangleSet->insertFaces(&_groupData->_geometry, faces, numCollisions);
	_triangleSet->load();
...
bool Group3D::hit(const RayGPUData& ray, float& distance, GLuint& modelCompID)
{
	...
	bvhCollisionShader->bindBuffers(std::vector<GLuint> { _staticGPUData->_clusterSSBO, _staticGPUData->_groupGeometrySSBO, _staticGPUData->_groupTopologySSBO, _staticGPUData->_groupMeshSSBO, raySSBO, collisionSSBO });
	bvhCollisionShader->use();
	bvhCollisionShader->setUniform("numClusters", _numClusters);
...
Group3D::GroupData* Group3D::aggregateSSBOData(VolatileGPUData*& volatileGPUData, StaticGPUData*& staticGPUData)
{
	...
	_groupData->_geometry.resize(numVertices);
	_groupData->_triangleMesh.resize(numTriangles);
	_groupData->_meshData.resize(_globalModelComp.size());

	std::cout << _groupData->_triangleMesh.size() << std::endl;
	...
	for (ModelComponent* modelComp : _globalModelComp)
	{
		...
		std::copy(modelComp->_geometry.begin(), modelComp->_geometry.end(), _groupData->_geometry.begin() + currentGeometry);
		std::copy(modelComp->_topology.begin(), modelComp->_topology.end(), _groupData->_triangleMesh.begin() + currentTopology);

		_groupData->_meshData[modelComp->_id]._startIndex = currentGeometry;
		_groupData->_meshData[modelComp->_id]._numVertices = numVertices;

		currentGeometry += numVertices;
		currentTopology += modelComp->_topology.size();

		if (false) modelComp->releaseMemory();
	}

	// Compute scene AABB once the geometry and topology is all given in a row
	_staticGPUData->_numTriangles		= _groupData->_triangleMesh.size();
	this->_aabb							= this->computeAABB(_groupData);

	staticGPUData->_groupGeometrySSBO	= ComputeShader::setReadBuffer(_groupData->_geometry, GL_STATIC_DRAW);
	staticGPUData->_groupMeshSSBO		= ComputeShader::setReadBuffer(_groupData->_meshData, GL_STATIC_DRAW);
	staticGPUData->_groupTopologySSBO	= ComputeShader::setReadBuffer(_groupData->_triangleMesh, GL_STATIC_DRAW);
...
AABB Group3D::computeAABB(GroupData* groupData)
{
