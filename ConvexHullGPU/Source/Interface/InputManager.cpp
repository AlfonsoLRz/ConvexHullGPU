...
const int InputManager::CALCULATE_COLLISIONS = GLFW_KEY_C;
...
bool InputManager::processPressedKeyEvent(const int key, const int mods)
{
...

	{
		switch(key)
		{
		case InputManager::CALCULATE_COLLISIONS:
			renderer->getCurrentScene()->computeCollisions();
...
void InputManager::mouseButtonEvent(GLFWwindow* window, int button, int action, int mods)
{
	...
	if (action == GLFW_PRESS && mods == GLFW_MOD_CONTROL)
	{
		...
		renderer->getCurrentScene()->hit(ray);
	}
}
